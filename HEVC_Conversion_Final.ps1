<#  Batch Convert to HEVC with FFMPEG using Nvidia graphics card or using your cpu
    This Script uses FFMPEG, If you do not have it you can download it here: https://ffmpeg.zeranoe.com/builds/
    Extract it to a location and update the $Encoder Variable to match the location
    The defaults For the conversion process are good but if you want better quality lower the number on line# 76 or 78
...
If (!( Test-Path -Path $Encoder )) {
    Write-Host "ffmpeg Not Found, Please Make Sure To Update Location In Script For Your Location" -ForegroundColor Black -BackgroundColor White
    Read-Host -Prompt "Press Enter To Exit Script"
    Break 
}
$Probe = 'C:\ffmpeg\bin\ffprobe.exe'
If (!( Test-Path -Path $Encoder )) {
    Write-Host "ffProbe Not Found, Please Make Sure To Update Location In Script For Your Location" -ForegroundColor Black -BackgroundColor White
    Read-Host -Prompt "Press Enter To Exit Script"
    Break 
}
